<script>
    ...
    const handlerSearch = () => {
        if(complaintVarStore.trigger){
            const q = query(colRef, orderBy("createdAt", "desc"), where("completeName", "==", complaintVarStore.kwiri))
            onSnapshot(q, (snapshots) => {
                let fbData = [];
                snapshots.docs.forEach(doc => {
                    let data = {...doc.data(), id: doc.id};
                    ...
                    <div class="fixed bottom-0 top-0 left-0 right-0 bg-white ">
                        ...
                        <div class="mx-auto max-w-[1000px] mt-[20vh] p-10">
                            ...
                            <div class="flex justify-center items-center">
                                <p class="w-full bg-[#272822] text-white p-2 border-2 font-bold">Date Recorded</p>
                                <p class="w-full bg-[#272822] text-white p-2 border-2 font-bold">Complainant Name</p>
                                <p class="w-full bg-[#272822] text-white p-2 border-2 font-bold">Complaint</p>
                                <p class="w-full bg-[#272822] text-white p-2 border-2 font-bold">Action Taken</p>
                                <p class="w-full bg-[#272822] text-white p-2 border-2 font-bold">Location Of Incidence</p>
                            </div>
